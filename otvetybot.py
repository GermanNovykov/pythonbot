...
api = API(merchant_id="1526526", merchant_key="rj6IEGhpd3tJsfMZhbCqtU8hNo5KIo8Z",
          server_callback_url="https://t.me/otvetynaizibot")
...
superuserusername = 'germannovykov'
# Chat timers
chat_timers = {}
post_timers = {}
# -------------------------------------
    ...
    if payload:
        ...
        if db.getcompleter(message.from_user.id):
            if db.getcompleter(message.from_user.id)[0][6] == 'yes':
                if message.from_user.id == post[1]:
                    ...
                    if db.postidchat(post[0]):
                        ...
                        await bot.send_message(post[1],
                                               f"Выполнитель {message.from_user.full_name} готов выполнить ваше задание {hide_link(post[10])}",
                ...
                await message.answer(
                    "Вы не можете брать посты, потому что администратор не рассмотрел вашей заявки или отклонил её")
        else:
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
            markup.add('Новый пост')
            markup.add('Стать выполнителем', 'Мои чаты', 'Мои посты', 'Мои деньги')
            await message.answer(
                "Чтобы брать посты, сначала нужно стать выполнителем\n\nНажмите «Стать выполнителем» для того чтобы оставить заявку",
                reply_markup=markup)
        ...
        if message.chat.type == 'private':
            ...
            await message.answer(f'Здравствуйте, это бот паблика «TurtleUA», выберите одно действие из меню.',
                                 reply_markup=markup)
...
# start2
@dp.message_handler(
    lambda message: message.text in ['Новый пост', 'Мои посты', 'Мои деньги', 'Мои чаты', 'Стать выполнителем'],
    state='*')
async def starthandlertwo(message: types.Message, state: FSMContext):
    if message.chat.type == 'private':
        ...
        if message.text == 'Новый пост':
            if db.finduserbyid(message.from_user.id)[0][3] <= 20:
                ...
                markup.add(item1, item2)
                msg = await bot.send_message(message.from_user.id, "Загрузка...",
                ...
                await bot.send_message(message.from_user.id, "Вы уже создали 20 публикаций сегодня")
        ...
        # becomecomp 1
            ...
            if not db.getcompleter(message.from_user.id):
                ...
                await bot.send_message(message.chat.id,
                                       ...
                                       reply_markup=markup, parse_mode=types.ParseMode.MARKDOWN_V2)
        ...
        # my bal 1
            ...
            if db.getcompleter(message.from_user.id):
                ...
                await message.answer(
                    f'На вашем балансе {completer[7]} грн\nПостов выполнено: {completer[8]}' if completer[
                        8] else f'На вашем балансе {completer[7]} грн', reply_markup=markup)
            else:
                await message.answer('У вас нету средств')

        elif message.text == 'Администрация':
            await message.answer(f"Администратор паблика:  <a href='https://telegram.me/{superuserusername}'>@{superuserusername}</a>", parse_mode=types.ParseMode.HTML)
# New Post 2
...
async def newpostprotection(callbackQuery: types.CallbackQuery, state: FSMContext):
    if callbackQuery.data == 'protected':
        ...
        await bot.edit_message_text('Укажите название предмета или тему задания', callbackQuery.message.chat.id,
...
async def newposttheme(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.from_user.id, f"Напишите краткое описание того, что нужно сделать",
                           reply_markup=markup)
    await NewPost.next()
        ...
        async with state.proxy() as data:
            if data["protection"] == 'protected':
                if message.text == "Договорная":
                    ...
                    data["docid"] = []
                    await bot.send_message(message.from_user.id,
                                           f"Теперь добавьте файл или фото, ассоциированый с заданием, затем нажмите «Готово»",
                                           reply_markup=markup)
                    await NewPost.next()
...
@dp.message_handler(MediaGroupFilter(is_media_group=False), content_types=['photo', 'document'],
                    state=NewPost.filehandle)
...
# New post 6
@dp.message_handler(MediaGroupFilter(is_media_group=True), content_types=['photo', 'document'],
                    state=NewPost.filehandle)
@media_group_handler
    ...
    async with state.proxy() as data:
        ...
        if message.text == "Отменить":
            ...
            if message.text == "Готово":
                ...
                post = Post(active="Активный", author=message.from_user.id, completer="Нету",
                            theme=data["theme"], maintext=data["maintext"], price=data["price"],
                            mediaid=data["medialink"], docid=data["doclink"], protection=data["protection"])
                data["post"] = post
                # message
        ...
        if message.text == 'Опубликовать':
            ...
            async with state.proxy() as data:
                ...
                if data["protection"] == "protected":
                    ...
                    markup.add(item1)
                    publishedmessage = await bot.send_message(publicationbotid, data['post'].tostring(),
                                                              reply_markup=markup, parse_mode=types.ParseMode.HTML)

                else:
                    publishedmessage = await bot.send_message(publicationbotid,
                                                              f"{data['post'].tostring()}\nКонтакт: <a href='https://telegram.me/{message.from_user.username}'>{message.from_user.first_name}</a>",
                                                              reply_markup=markup, parse_mode=types.ParseMode.HTML,
                ...
                msgtoadmin = await bot.send_message(supergroupid, data["post"].tostring(),
                                                    parse_mode=types.ParseMode.HTML, reply_markup=markup2)

                # linkhandle
                linktopublished = publishedmessage.url
                db.givepostalink(postid, linktopublished)
                db.giveadmlink(postid, msgtoadmin.message_id)
            # user
            post_timers[postid] = asyncio.get_event_loop().time() + 900  # 15 minutes
            ...
            await bot.send_message(message.from_user.id,
                                   f"Отлично, пост готов и опубликован на основном канале {hide_link(linktopublished)}",
                                   reply_markup=markup2, parse_mode=types.ParseMode.HTML)
            await state.finish()
            db.increaseuserpsot(message.from_user.id)
            await asyncio.sleep(86400)
            db.resetuserposts(message.from_user.id)
...
async def mypostsshow(call: types.CallbackQuery, state: FSMContext):
    ...
    item3 = types.InlineKeyboardButton('Обновить пост на канале', callback_data='obnovitpost')
    markup.add(item3)
    markup.add(item1, item2)
    ...
    await bot.edit_message_text(
        ...
        call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode=types.ParseMode.HTML)
    await MyPosts.next()


# My posts 3
@dp.callback_query_handler(lambda call: call.data in ['postdelete', 'postback', 'reglarpost', 'obnovitpost'],
                           state=MyPosts.deleteorback)
async def mypostsdelete(call: types.CallbackQuery, state: FSMContext):
        ...
        if db.findallposts(call.message.chat.id):
            ...
            for i in posts:
                if i[2] == 'Активный':
                    ...
                    n += 1
            await MyPosts.choice.set()
            await bot.edit_message_text('Выберите одну публикацию', call.message.chat.id, call.message.message_id,
                                        reply_markup=markup)
        else:
    ...
    elif call.data == 'obnovitpost':
        async with state.proxy() as data:
            post = data['post']
        if post[0] in post_timers:
            timer = post_timers[post[0]]
            remaining_time = timer - asyncio.get_event_loop().time()

            # If less than 15 minutes have passed, restrict the action
            if remaining_time > 0:
                await bot.delete_message(call.message.chat.id, call.message.message_id)
                await bot.send_message(call.message.chat.id, f"Подождите {remaining_time:.0f} секунд перед тем как ещё раз обновить пост.")
                return
        #update links
        post_timers[post[0]] = asyncio.get_event_loop().time() + 900  # 15 minutes
        doclinks = eval(post[9])
        postinst = Post(post[1], post[2], post[3], post[4], post[5], post[6], post[7], post[8], doclinks)

        message_id = re.search(r'/(\d+)$', post[10]).group(1)
        await bot.delete_message(publicationbotid, message_id)

        link = await get_start_link(str(post[0]), encode=True)
        markup = types.InlineKeyboardMarkup()
        if post[4] == "protected":
            item1 = types.InlineKeyboardButton('Беру', url=link)
            markup.add(item1)
            publishedmessage = await bot.send_message(publicationbotid, postinst.tostring(),
                                                      reply_markup=markup, parse_mode=types.ParseMode.HTML)
            ...
            url = f"https://telegram.me/{call.from_user.username}"
            markup2 = types.InlineKeyboardMarkup()
            item2 = types.InlineKeyboardButton('Удалить', callback_data=f"admindelete{encoded_data}")
            item3 = types.InlineKeyboardButton('Контакт пользователя', url=url)
            markup2.add(item2, item3)
            await bot.delete_message(supergroupid, int(post[12]))
            msgtoadmin = await bot.send_message(supergroupid, postinst.tostring(),
                                                parse_mode=types.ParseMode.HTML, reply_markup=markup2)

            db.giveadmlink(post[0], msgtoadmin.message_id)
            db.givepostalink(post[0], publishedmessage.url)
        else:
            publishedmessage = await bot.send_message(publicationbotid,
                                                      f"{postinst.tostring()}\nКонтакт: <a href='https://telegram.me/{call.message.from_user.username}'>{call.message.from_user.first_name}</a>",
                                                      reply_markup=markup, parse_mode=types.ParseMode.HTML,
                                                      disable_web_page_preview=True)
            # adminpart
            encoded_data = urllib.parse.urlencode({
            ...
            markup2.add(item2, item3)
            await bot.delete_message(supergroupid, int(post[12]))
            msgtoadmin = await bot.send_message(supergroupid, f"{postinst.tostring()}\nКонтакт: <a href='https://telegram.me/{call.message.from_user.username}'>{call.message.from_user.first_name}</a>",
                                                parse_mode=types.ParseMode.HTML, reply_markup=markup2, disable_web_page_preview=True)

            db.giveadmlink(post[0], msgtoadmin.message_id)
            db.givepostalink(post[0], publishedmessage.url)

        await bot.delete_message(call.message.chat.id, call.message.message_id)
        await bot.send_message(call.message.chat.id, f"Пост заново опубликован на основном канале {hide_link(publishedmessage.url)}", parse_mode=types.ParseMode.HTML)

# I take post 2
@dp.callback_query_handler(lambda call: call.data.startswith('takeapprove') or call.data.startswith('takenot'),
                           state='*')
    ...
    if call.data.startswith('takeapprove'):
        ...
        if post[11]:
            ...
            if db.getfreechat():
                ...
                markup.add(item1)
                userinvite = await bot.edit_message_text('***Вы приняли заявку***\n\nТеперь вы можете открыть чат',
                                                         call.message.chat.id, call.message.message_id,
                                                         reply_markup=markup, parse_mode=types.ParseMode.MARKDOWN_V2)
                completerinvite = await bot.send_message(completer,
                                                         f"<b>Пользователь</b> принял вашу заявку {hide_link(postlink)}",
                ...
                #if chat is still active
                if db.chat_byid(freechat)[0][4] == post[0]:
                    inlinemark = types.InlineKeyboardMarkup()
                    items1 = types.InlineKeyboardButton("Продлить на сутки", callback_data="unusedchatextend")
                    items2 = types.InlineKeyboardButton("Отменить сделку", callback_data="chatdealrefuse")
                    inlinemark.add(items1, items2)
                    await bot.send_message(freechat,
                                       "Чат используется уже 3 дня. Продолжить сделку или завершить? Чат автоматически будет удалён через 24 часа", reply_markup=inlinemark)
        ...
        await bot.edit_message_text('Вы отклонили заявку на выполнение работы.', call.message.chat.id,
                                    call.message.message_id)
        await bot.send_message(completer,
                               f"Пользователь отклонил вашу заявку на выполнение работы {hide_link(postlink)}",
                               parse_mode=types.ParseMode.HTML)
...
async def welcome_new_user(message: types.Message):
    ...
    if message.chat.id in integer_list:
        ...
        if types.ChatMember.is_chat_member(completer_member) and types.ChatMember.is_chat_member(user_member):
            medialinks = post[8]
            doclinks = eval(post[9])
            chat_timers[completer[1]] = asyncio.create_task(start_timer(message.chat.id, completer[1]))
            chat_timers[user] = asyncio.create_task(start_timer(message.chat.id, user))
...
@dp.callback_query_handler(
    lambda call: call.data.startswith('pozvatklienta') or call.data.startswith('pozvatcompleter'), state='*')
...
async def chatdealrefuse(call: types.CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        data['caller'] = call.from_user.id

    await bot.send_message(call.message.chat.id,
                           "***Вы уверены, что вы хотите отменить сделку?***\n\nНапишите: `Да, я хочу отменить сделку`\n\nТекст можно скопировать, нажав на него\n***Важно*** После отмены чат будет удалён",
                           parse_mode=types.ParseMode.MARKDOWN_V2)
...
async def becomecomponephone(message: types.Message, state: FSMContext):
    if message.text == "Отменить":
        ...
        await bot.send_message(message.from_user.id,
                               "***Введите номер телефона***\n\nНажмите кнопку ***Поделится номером телефона***",
                               reply_markup=markup, parse_mode=types.ParseMode.MARKDOWN_V2)
        await BecomeCompleter.next()
...
async def handle_contact(message: types.Message, state: FSMContext):
    if message.text == "Отменить":
        ...
        await bot.send_message(message.from_user.id, "Чтобы оставить заявку, нажмите ***Подтверить***",
                               reply_markup=markup, parse_mode=types.ParseMode.MARKDOWN_V2)
        await BecomeCompleter.next()
...
async def becomecomponephone(message: types.Message, state: FSMContext):
    if message.text == "Отменить":
        ...
        async with state.proxy() as data:
            ...
            await bot.send_message(superuserid,
                                   f"Заявка от <a href='{url}'>{message.from_user.first_name}</a>\n\nДанные\n<b>Настоящее имя: </b>{data['name']}\n<b>Почта: </b>{data['email']}\n<b>Дата рождения: </b>{data['date']}\n<b>Номер телефона: </b>{data['phone']}",
                                   reply_markup=markup, parse_mode=types.ParseMode.HTML)
        await bot.send_message(message.from_user.id,
                               "Ваша заявка принята и будет рассмотрена администратором. После этого вы сможете выполнять публикации клиентов",
...
@dp.callback_query_handler(
    lambda call: call.data.startswith('compstatusaccept') or call.data.startswith('compstatusreject'))
async def admincallll(call: types.CallbackQuery, state: FSMContext):
    if call.data.startswith('compstatusaccept'):
        ...
        await bot.send_message(completer,
                               "Администратор принял вашу заявку. Теперь вы можете выполнять публикации клиентов")
...
async def pay(call: types.CallbackQuery, state: FSMContext):
    ...
    if type(post[7]) == float:
        ...
        if result.get('response') and 'error_message' in result['response']:
            ...
            markup.add(item1, item2)
            await bot.edit_message_text(
                f"{db.finduserbyid(chat[3])[0][2]}, чтобы оплатить сделку, вам нужно нажать кнопку <b>Оплатить</b> и выбрать удобный способ оплаты\n\nПосле успешной оплаты прийдёт уведомление и {db.getcompleter(chat[2])[0][2]} может начать выполнять задание\n\n<b>К оплате:</b> {price} грн\n<b>Комиссия: 5 грн</b>",
                call.message.chat.id, call.message.message_id, parse_mode=types.ParseMode.HTML, reply_markup=markup)
        ...
        await call.message.answer("Нажмите ***Поменять цену*** и укажите новую цену",
                                  parse_mode=types.ParseMode.MARKDOWN_V2)
...
async def mychatback(call: types.CallbackQuery):
    ...
    if chats:
        ...
        for i in chats:
            ...
            n += 1
        await MyChats.chatschoice.set()
        await bot.edit_message_text('Выберите один чат', call.message.chat.id, call.message.message_id,
                                    reply_markup=markup)
    else:
...
async def changeprice(call: types.CallbackQuery):
    ...
    await bot.send_message(call.message.chat.id,
                           f"<b>СТАРАЯ ЦЕНА:</b> {f'{int(post[7])} грн' if type(post[7]) == float else 'Договорная'}\nНазвите новую цену или нажмите <b>Отменить</b>\n\nНапример, чтобы указать цену в 100 грн, введите «100»",
                           reply_markup=markup, parse_mode=types.ParseMode.HTML)
...
async def changingp(message: types.Message, state: FSMContext):
    ...
    if message.from_user.id == chat[3]:
        if message.text == "Да, я абсолютно уверен":
            ...
            message_id = re.search(r'/(\d+)$', post[10]).group(1)
            await bot.edit_message_text(postinst.tostring(), publicationbotid, message_id,
                                        parse_mode=types.ParseMode.HTML)
...
async def withcheck(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(message.chat.id,
                           f'{md.bold("Заявка на вывод денег отправлена")}\n\nАдминистратор проверит заявку в течении 24 часа и отправит деньги на карту',
                           parse_mode=types.ParseMode.MARKDOWN_V2)
    await bot.send_message(superuserid,
                           f"<b>Заявка на вывод денег от {completer[2]}</b>\nДанные выполнителя: {completer}\n\nУказанная карта для перевода: {message.text}",
                           parse_mode=types.ParseMode.HTML, reply_markup=markup)
    await state.finish()


@dp.callback_query_handler(lambda call: call.data.startswith('withsuccess'))
async def oplacheno(call: types.CallbackQuery):
    encodeddata = call.data[11:]
    decoded_data = urllib.parse.parse_qs(encodeddata)
    ...
    await bot.edit_message_text(
        f"{completer[2]} оплата завершена\nНовый баланс выполнителя: {db.getcompleter(decoded_data['completerid'][0])[0][7]}",
        call.message.chat.id, call.message.message_id)



@dp.callback_query_handler(lambda call: call.data.startswith('unusedchatextend'))
async def unusedchatextend(call: types.CallbackQuery):
    await bot.delete_message(call.message.chat.id, call.message.message_id)
    await bot.send_message(call.message.chat.id, "Сделка продлена на сутки")

async def timeout_handler(chat_id, user_id):
    # Implement your desired action when there is no message within 15 seconds
    chat = db.chat_byid(chat_id)[0]
    if user_id == chat[2]:
        # napisat adminy, zakrit sdelky
        await bot.send_message(chat_id, f"Выполнитель не отвечал в течении 24 часов, чат будет закрыт, средства будут возвращены заказчику. {chat_id}.")
        encoded_data = urllib.parse.urlencode({
            "user": user_id,
            "chatid": chat_id,
            # "text": f"Выполнитель <b>{db.getcompleter(chat[2])[0][2]}</b> не отвечал в течении 24 часов"
        })
        invitelink = await bot.create_chat_invite_link(chat_id)
        markup = types.InlineKeyboardMarkup()
        item1 = types.InlineKeyboardButton("Закрыть чат", callback_data=f"zakritunusedcomp{encoded_data}")
        item2 = types.InlineKeyboardButton("Перейти в чат", url=invitelink.invite_link)
        markup.add(item1, item2)
        await bot.send_message(superuserid, f"Выполнитель <b>{db.getcompleter(chat[2])[0][2]}</b> не отвечал в течении 24 часов", reply_markup=markup, parse_mode=types.ParseMode.HTML)
    elif user_id == chat[3]:
        encoded_data = urllib.parse.urlencode({
            "user": user_id,
            "chatid": chat_id,
            # "text": f"Заказчик <b>{db.finduserbyid(user_id)[0][2]}</b> не отвечал в течении 24 часов, чат будет закрыт, средства будут засчитаны выполнителю."
        })
        # napisast adminy o vozvrate, zakrit sdelky
        invitelink = await bot.create_chat_invite_link(chat[1])
        markup = types.InlineKeyboardMarkup()
        item1 = types.InlineKeyboardButton("Закрыть чат", callback_data=f"zakritunuseduser{encoded_data}")
        item2 = types.InlineKeyboardButton("Перейти в чат", url=invitelink.invite_link)
        markup.add(item1, item2)
        await bot.send_message(chat_id, f"Заказчик <b>{db.finduserbyid(user_id)[0][2]}</b> не отвечал в течении 24 часов, чат будет закрыт, средства будут засчитаны выполнителю.", parse_mode=types.ParseMode.HTML)
        await bot.send_message(superuserid, f"Заказчик <b>{db.finduserbyid(user_id)[0][2]}</b> не отвечал в течении 24 часов, чат будет закрыт, средства будут засчитаны выполнителю. {chat_id}.", parse_mode=types.ParseMode.HTML, reply_markup=markup)


# Define a function to start the timer for a chat
async def start_timer(chat_id, user_id):
    # Wait for 15 seconds
    await asyncio.sleep(20)
    # Trigger the timeout handler if no message is received within 15 seconds
    await timeout_handler(chat_id, user_id)

# Define a handler for incoming messages
@dp.message_handler()
async def unusedtimeouts(message: types.Message):
    if message.chat.id in chatbotids and not message.is_command():
        ...
        # Start a new timer
        chat_timers[user_id] = asyncio.create_task(start_timer(message.chat.id, user_id))

@dp.callback_query_handler(lambda call: call.data.startswith('zakritunusedcomp') or call.data.startswith('zakritunuseduser'), state='*')
async def zakritunused(call: types.CallbackQuery):

    encodeddata = call.data[16:]
    decoded_data = urllib.parse.parse_qs(encodeddata)
    chatid = decoded_data["chatid"][0]
    if call.data.startswith('zakritunusedcomp'):

        markup = types.InlineKeyboardMarkup()
        item1 = types.InlineKeyboardButton("Подтвердить", callback_data=f"zakritunusedpodtvcomp{encodeddata}")
        item2 = types.InlineKeyboardButton("Назад", callback_data=f"zakritunusednazad{encodeddata}")
        markup.add(item1, item2)
        await bot.edit_message_text("Подтвердить закрытие сделки?", call.message.chat.id, call.message.message_id,
                                    reply_markup=markup)

    elif call.data.startswith('zakritunuseduser'):
        markup = types.InlineKeyboardMarkup()
        item1 = types.InlineKeyboardButton("Подтвердить", callback_data=f"zakritunusedpodtvuser{encodeddata}")
        item2 = types.InlineKeyboardButton("Назад", callback_data=f"zakritunusednazad{encodeddata}")
        markup.add(item1, item2)
        await bot.edit_message_text("Подтвердить закрытие сделки?", call.message.chat.id, call.message.message_id,
                                    reply_markup=markup)
@dp.callback_query_handler(lambda call: call.data.startswith('zakritunusedpodtvcomp') or call.data.startswith('zakritunusednazad') or call.data.startswith('zakritunusedpodtvuser'), state='*')
async def zakritpodverdit(call: types.CallbackQuery):
    # if user doesnt respond, disable his post
    # if completer doesnt respond, retain post but refund user, which would be discussed with user, close the chat
    if call.data.startswith('zakritunusedpodtvcomp'):
        await bot.edit_message_text("Сделка закрыта", call.message.chat.id, call.message.message_id)

        encodeddata = call.data[21:]
        decoded_data = urllib.parse.parse_qs(encodeddata)
        chatid = decoded_data["chatid"][0]
        chat = db.chat_byid(chatid)[0]

        #process that completer did not provide solution, so if money was paid, money should go back to user


        #chatdeletion
        db.clearpostchatid(chat[4])
        db.clear_chat(chat[1])

        await bot.delete_message(chat_id=chat[2], message_id=chat[6])
        await bot.delete_message(chat_id=chat[3], message_id=chat[5])

        await bot.unpin_all_chat_messages(chat[1])

        if chat[2] != superuserid:
            await bot.kick_chat_member(chat[1], chat[2])
            await bot.unban_chat_member(chat[1], chat[2])
        if chat[3] != superuserid:
            await bot.kick_chat_member(chat[1], chat[3])
            await bot.unban_chat_member(chat[1], chat[3])

    elif call.data.startswith('zakritunusedpodtvuser'):
        await bot.edit_message_text("Сделка закрыта", call.message.chat.id, call.message.message_id)
    ...
    elif call.data.startswith('zakritunusednazad'):
        encodeddata = call.data[17:]
        decoded_data = urllib.parse.parse_qs(encodeddata)
        chatid = decoded_data["chatid"][0]
        userid = decoded_data["user"][0]
        chat = db.chat_byid(chatid)[0]
        invitelink = await bot.create_chat_invite_link(chat[1])
        if chat[2] == int(userid):
            mstext = f"Выполнитель <b>{db.getcompleter(chat[2])[0][2]}</b> не отвечал в течении 24 часов"
            markup = types.InlineKeyboardMarkup()
            item1 = types.InlineKeyboardButton("Закрыть чат", callback_data=f"zakritunusedcomp{encodeddata}")
            item2 = types.InlineKeyboardButton("Перейти в чат", url=invitelink.invite_link)
            markup.add(item1, item2)

            await bot.edit_message_text(mstext, call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode=types.ParseMode.HTML)

        elif chat[3] == int(userid):
            mstext = f"Заказчик <b>{db.finduserbyid(userid)[0][2]}</b> не отвечал в течении 24 часов, чат будет закрыт, средства будут засчитаны выполнителю."
            markup = types.InlineKeyboardMarkup()
            item1 = types.InlineKeyboardButton("Закрыть чат", callback_data=f"zakritunuseduser{encodeddata}")
            item2 = types.InlineKeyboardButton("Перейти в чат", url=invitelink.invite_link)
            markup.add(item1, item2)
